package AffiliationURLGenerator;
...
sub new {
    ...
    $self->{levier} = 'Affiliation'; # Définit le nom du canal
    return $self;
}
...
sub valider_parametres {
    my ($self, $params) = @_;
    my @parametres_obligatoires = qw(domaine_tracking site nom_support nom_campagne nom_banniere format_banniere url_destination);
...
# Genera las URLs para el canal Affiliation
sub generer_urls {
    my ($self, $params) = @_;
    ...
    my $url_parametres = sprintf(
        ...
        $params->{url_destination}, $params->{nom_support}, $params->{nom_campagne}, $params->{nom_support},
        $params->{nom_banniere}, $params->{format_banniere}, $params->{format_banniere},
    ...
    my $url_impression = sprintf(
        ...
        $params->{domaine_tracking}, $params->{site}, $params->{nom_support}, $params->{nom_campagne}, $params->{nom_support},
        $params->{nom_banniere}, $params->{format_banniere}, $params->{format_banniere},
        $params->{nom_segment} // '', $params->{valeur_segment} // '', $params->{nom_plan_medias} // '',
        $params->{adid} // '', $params->{idfa} // ''
    );
...
# Traite le CSV pour le levier Affiliation
sub process_csv {
    my ($self) = @_;

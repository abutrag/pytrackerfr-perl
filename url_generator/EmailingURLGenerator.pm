...
sub new {
    ...
    $self->{levier} = 'Emailing'; # Définit le nom du canal
    return $self;
}
...
sub valider_parametres {
    my ($self, $params) = @_;
    my @parametres_obligatoires = qw(domaine_tracking site nom_support nom_campagne url_destination);
...
sub generer_urls {
    ...
    my $url_parametres = sprintf(
        ...
        $params->{id_client} // '', $params->{adid} // '', $params->{idfa} // ''
    );

    # Ajoute mail_utilisateur si présent
    if ($params->{mail_utilisateur}) {
        $url_parametres .= sprintf("&eemail=%s", $params->{mail_utilisateur});
    }
...
# Traite le CSV pour le levier Emailing
sub process_csv {
    my ($self) = @_;
